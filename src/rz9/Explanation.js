...
export default function Explanation() {
  return (
    <div style={{ maxWidth: 900, margin: "24px auto", lineHeight: 1.6 }}>
      <h2 style={{ marginTop: 0 }}>Rankings Details</h2>

      <p><strong>Parameters:</strong> H (half-life, days), K (step size), μ (default rating), W (Elo width), ν (neutral rate).</p>
      ...
      <BlockMath math={`R_0 = R_{\\text{init}},\\quad R_i = R_{i-1} + K\\, w_i \\,\\big(r_i - E(R_{i-1})\\big),\\quad i=1,\\dots,n.`} />

      <BlockMath math={`R^{\\ast} = R_n.`} />

      <p><em>Leaderboard:</em> sort by <strong>R<sup>*</sup></strong> (desc). Tiebreak by total score rate.</p>
    </div>
  );
}
